...
class Musisi {
  ...
  constructor(nama, laguHits, genre) {
    ...
    (this.nama = nama), (this.lagu = laguHits), (this.genre = genre);
  }
}
...
function displayData(dataKu) {
  ...
  for (var i = 0; i < dataKu.length; i++) {
    hasil += `
        ${i + 1}. Warna : ${dataKu[i].colour}, Tipe : ${
      dataKu[i].type
    }, Brand : ${dataKu[i].brand}, Tahun : ${dataKu[i].year}`;
  }
...
function filtering(
  ...
  for (var i = 0; i < arr.length; i++) {
    if (
      arr[i].colour.includes(filterWarna) &&
      arr[i].type.includes(filterTipe) &&
      arr[i].brand.includes(filterMerek) &&
      arr[i].year.includes(filterTahun)
    ) {
      newArr.push(arr[i]);
    }
  }
...
console.log(filtering(data, "Merah", "", "", ""));
